...
inheritance-based ("OOP") architecture — with numbers, not just claims.
...
Traditional game object architectures attach data *and* behavior to a class (`Player : GameObject`,
`Enemy : GameObject`), relying on inheritance and virtual methods. This is intuitive but has two
costs at scale: inheritance hierarchies get awkward as behaviors mix, and object data is scattered
across the heap — iterating thousands of objects means jumping around memory unpredictably (cache
misses).

ECS splits data and behavior apart entirely:
...
arrays instead of scattered heap objects. This is the pattern behind Unity DOTS and most modern
...
SpatialHashGrid  = buckets entities into fixed-size grid cells for fast "who's nearby" queries
```

Component storage progressed through two implementations, each solving a different cost:

1. **`unordered_map<Entity, T>`** (initial) — correct, but every `get()` is a hash lookup.
...
2. **Spatial hash grid** — entities bucketed into grid cells each frame; each entity only checked
   against the ~9 cells (its own + 8 neighbors) around it, avoiding the vast majority of
   irrelevant pairwise checks.

## Benchmark

All three variants run the identical simulation (same entity count, same RNG seed, same movement/
wall-bounce/collision logic) so the only variable is architecture. Rendering is excluded from
timing — only the update step (movement + wall bounce + collision) is measured, using
...
quadratic, as expected for O(n²) pairwise checks); the spatial-grid curve stays close to linear.

### The naive ECS result is slower than OOP — and that's worth explaining, not hiding
